...
| **NIC link speed** | negotiated speed/duplex per camera-facing wired NIC from `/sys/class/net/<if>/speed`, judged against what the **link partner advertised** (`ethtool`) — a gigabit switch port offering 1000baseT while the link sits at **100 Mb/s** (or 10, or half-duplex) is the signature of a damaged cable, bad crimp/patch panel, flaky switch port or failed auto-neg, caught **before** packets are lost. A camera cabled straight to the host, or a Fast-Ethernet switch, offers 100 at most, so 100 there is correct and quiet: no topology knowledge needed. Published as `nic_link` (per interface, with `partner_max_mbps` / `expected_mbps`) + `nic_link_speed_mbps` (the slowest watched link). Wifi / down links have no speed and never fire; a link whose partner advertisement can't be read (no `ethtool`, driver doesn't report it) is noted in `nic_link_note` and not judged. |
| **Uptime** | host uptime |
...
| `timelapse_overdue` | degraded | saving frames but no `_frames_timelapse.mp4` (the day-frames timelapse in `FramesFiles/`, **not** the night `CapturedFiles` timelapse) produced in ages — generation not running at all; latitude-independent | `timelapse_max_age_s` (30h) |
| `log_fatal` | error | `Traceback`/`ImportError`/`cannot open shared object`/`undefined symbol`/segfault/Cython-build-failure in the RMS log **or** the capture's systemd-journal tail (the latter catches startup crashes that die before RMS logging starts). Loader lines (`cannot open shared object`, `undefined symbol`) are **ignored at INFO/DEBUG level** — RMS pipes GStreamer/GLib through its logger at INFO, and GStreamer's probes for *optional* plugins (e.g. the CUDA plugin looking for `libnvrtc.so` on a box with no NVIDIA stack) log exactly that wording while nothing is broken | — |
...
| `nic_link_slow` | degraded | a camera-facing wired NIC negotiated **below what its link partner advertised** (e.g. 100 Mb/s on a gigabit switch port) — cable / crimp / switch port / auto-neg; named per interface. **Half duplex always fires** (a partner that isn't auto-negotiating = forced port / duplex mismatch) | `nic_link_speed_min_mbps` (`auto` = judge against the partner; a number is a hard floor in Mb/s instead; 0 = off) |
