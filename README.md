...
| **NIC link speed** | negotiated speed/duplex per camera-facing wired NIC from `/sys/class/net/<if>/speed` — a gigabit port that has auto-negotiated down to **100 Mb/s** (or 10, or half-duplex) is the signature of a damaged cable, bad crimp/patch panel, flaky switch port or failed auto-neg, caught **before** packets are lost. Published as `nic_link` (per interface) + `nic_link_speed_mbps` (the slowest watched link). Wifi / down links have no speed and never fire. |
| **Uptime** | host uptime |
...
| `nic_link_slow` | degraded | a camera-facing wired NIC negotiated **below its expected link speed** (e.g. 100 Mb/s on a gigabit port) — cable / crimp / switch port / auto-neg; named per interface, half-duplex called out | `nic_link_speed_min_mbps` (1000; set 100 on a Fast-Ethernet host, 0 = off) |
